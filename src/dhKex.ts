...
function verifyKey(host_key: any, sig: any) {
  var rsa = new SSHyClient.RSAKey(new SshClientMessage(host_key));
  if (!rsa.verify(SshClientKex.H, new SshClientMessage(sig))) {
    (window as any).transport.disconnect();
    throw 'RSA signature verification failed, disconnecting.';
  }
}

export class SshClientKexDiffieHellman {
  p: any;
  q: any;
  g: any;
  x: any;
  e: any;
  f: any;
  minBits: number | undefined;
  maxBits: number | undefined;
  preferredBits: number | undefined;
  P: BigInteger | undefined;
  G: BigInteger | undefined;

  constructor(public transport: SshClientTransport, public group: any, public SHAVersion: any) {
  ...
  handleDHReply(r) {
    ...
    r = new SshClientMessage(r);

    // get the host key, f and signature from the message
    var host_key = r.get_string();
    if ((window as any).transport.settings.rsaCheckEnabled) {
    ...
    */
    ...
    SshClientKex.K = K;
    SshClientKex.sessionId = SshClientKex.H = this.SHAVersion == 'SHA-1' ? new SSHyClient.hash.SHA1(m.toString()).digest() : new SSHyClient.hash.SHA256(m.toString()).digest();
    if ((window as any).transport.settings.rsaCheckEnabled) {
