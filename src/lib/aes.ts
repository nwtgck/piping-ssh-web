...
class aes {
  ...
  encrypt(e: Uint8Array, d: number[], c: any) {
    d = this.mode == sjcl.cipher.aes.MODE_CBC ? d : [];
    for (var b: any[] = [], a = 0; a < e.length / 16; a++) {
  ...
  decrypt(e: Uint8Array, d: number[]) {
    for (var c: any[] = [], b = 0; b < e.length /
    16; b++) {
  ...
  _precompute() {
    var e: any[] = this._tables[0],
      d: any[][] = this._tables[1],
      c: any = e[4],
      b: any = d[4],
      a, f, g, h = [],
      n = [],
...
export var sjcl = {
  ...
  bitArray: {
    bitLength: function (e: number[]) {
      var d = e.length;
      return 0 === d ? 0 : 32 * (d - 1) + sjcl.bitArray.getPartial(e[d - 1])
    },
    partial: function (e: number, d: number, c?: true) {
      return 32 === e ? d : (c ? d | 0 : d << 32 - e) + 1099511627776 * e
    },
    getPartial: function (e: number) {
